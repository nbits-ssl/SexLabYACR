...
* フォロワー管理にNFFを使用する場合、「フォロワー数の常時カウント」をオフに
  してください。オンの場合YACR発動時にインポートフォロワーが外れます。
...
* Nether's Follower Framework (昔は FollowerLivePackage)
* Loot and Degradation
* Armor Rating Rescaled - SKSE
* Action Combat 1.5
* Combat Evolvedの設定値消去版
...
* SE用のこまごま処理
* 設定ファイルのセーブ・ロード機能を追加
* 裸判定の部分だけをいじれるように裸判定部を別スクリプトファイル化(YACRExtraNaked.psc)
  MCMで「裸判定に外部スクリプト使用」をONにすると有効に
