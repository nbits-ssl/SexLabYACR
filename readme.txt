...
SEで製作/動作確認をしていますが、多分LEでも問題なく動きます。


## 概要

* 防御せずに攻撃を受けた時に、MCMの設定に応じた条件・確率でレイプが発
  動します。
...
* フォロワーでないNPCは認識しません。
* NPCに同様の機能をさせたい場合は SexLab YACR for NPC を併用してください。
...
* フォロワー管理にNFFを使用する場合で「インポートフォロワー」がいる場合、
、「フォロワー数の常時カウント」をオフにしてください。
  オンの場合、YACR発動時にインポートフォロワーが外れます。インポートフォ
  ロワーを使わない場合はオンでも構いません。
...
* エクスプロージョンなど直接当たらなくても爆風でダメージを受ける攻撃手段を
  持つ敵を相手にしている場合、遠距離攻撃を不許可にしても、爆風に当たった際
  に敵が近くにいるとYACRが発動する可能性があります。
...
* 戦闘バランス調整系との競合は(自環境では使ってないため)わかりません。
  が、公開からかなり時間が経っても、例えばUltimate Combatと競合した、という
  話は聞きませんので大きな問題はないのかなと思います。
...
プロファイル機能によるセーブ・ロードでは、これらの値も対象とします。
...
* 今まで2,000固定だったレイプ時に観衆を呼び寄せる距離をMCMで設定可能に
 (多く呼び寄せた方が複数プレイにはなりやすくなりますが、脱出の困難さは上がります)
...
* 大治癒など非敵対呪文にもYACRが適用されていたのを修正、が判定だけ入れてキャンセル
  処理が漏れていたのを修正


### 2019/06/09 - 2.7

* 大治癒など非敵対呪文にもYACRが適用されていたのを修正
...
* We Can't Die用の内部的な機能追加
* 群衆の入れ替えタイミングを毎ステージごとから毎ループごとに変更

* 「PCはノックダウンのみ」が初期設定ONになっていたのを修正(Thanks bbspink)
* 3P時の人間を検出するクエストが人間以外も検出していたのを修正
* フォロワーノックダウン時、相手を探す処理がスキップされる場合があったのを修正

* 自環境のフォロワー管理ModがFLPからNFFに変わったため、Readmeを修正
　(NFFを使用する場合、「フォロワー数の常時カウント」をオフにしてください)


### 2019/04/17 - 2.5

* SexLab Util1を被害者側のみ限定的にサポート(デフォルトOFF)
* 2.1時に実装したSSLの絶頂イベント送信をON/OFF可能に(デフォルトON)
* SexistGuards導入時、「抵抗する」成功時にダイアログループを停止するように
  (Thanx! bbspink 106.407)

* トロールの攻撃の最大リーチが「近接攻撃」の判定範囲外だったのを修正
* エンドレスレイプで「第2ステージから再度」が機能していなかったのを修正
* MCMとReadmeを少し整理


### 2019/01/24 - 2.4

* PCノックダウン時強制的に3人称へ変更するように修正
